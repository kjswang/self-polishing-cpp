...
double dist_nullspace_arma(vec x0, mat theta0, mat nullspace, mat DH, mat base, capsule_arma cap[], tool_arma tl, mat PC, double alpha) {
    ...
    mat point1, point2;
    double dist, radius;
    std::vector<double> distList(sizeof(pos));
...
bool check_ineq_need(MatrixXd theta, MatrixXd DH, MatrixXd base, capsule cap[], double consider_line){
    ...
    for (int i = 0; i < nlink; i++) {
        cout << pos[i].p1(0,0) << pos[i].p2(0,0) << endl;
...
bool checkFeasible(MatrixXd theta, MatrixXd PC, MatrixXd PC_idx, MatrixXd DH, MatrixXd base, capsule RoCap[], tool tl){
    ...
    for (int i = 0; i < sizeof(pos); i++) {
        ...
        //if (inhull(point1.transpose(), PC_hull) == 0 && point1(0) > (PC_minX + offset)) {
            feasible = false;
            return feasible;
        }
...
// if (file.is_open()) {
//     for (int i = 0; i < safe_theta.rows(); ++i) {
//         for (int j = 0; j < safe_theta.cols(); ++j) {
//             file << safe_theta(i, j) << " ";
//         }
//         file << "\n";
//     }
...
// }
cout << safe_wp_pos << endl;
std::ofstream file("safe_wp_pos_test.txt"); // Open a file for writing

if (file.is_open()) {
    for (int i = 0; i < safe_wp_pos.rows(); ++i) {
        for (int j = 0; j < safe_wp_pos.cols(); ++j) {
            file << safe_wp_pos(i, j) << " ";
        }
        file << "\n";
    }
    file.close();
